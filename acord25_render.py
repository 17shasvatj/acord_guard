...
def render_acord25(path: str, fields: dict, receipts: list[dict],
    ...
    # supports. Match on CONTAINS, not equality, so natural model phrasing like
    # "Occurrence", "occurrence form", "per policy" resolves correctly. If
    # neither was verified from the source, both stay empty rather than assert an
    # unverified form. Same discipline as every other field.
    trig = str(fields.get("coverage_trigger", "")).strip().casefold()
    cm = "[X]" if "claims" in trig else "[ ]"
    oc = "[X]" if "occurrence" in trig else "[ ]"
    tiny(c1 + 10, y8 - 20, f"{cm} CLAIMS-MADE   {oc} OCCUR")
    # Aggregate basis (policy / project / loc): same rule, same contains-match.
    agg = str(fields.get("aggregate_basis", "")).strip().casefold()
    ap = "[X]" if "policy" in agg else "[ ]"
    apr = "[X]" if "project" in agg else "[ ]"
    al = "[X]" if ("loc" in agg or "location" in agg) else "[ ]"
